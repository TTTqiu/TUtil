...
class MemoryCacheUtil {
    ...
    static void putBitmapToMemory(String key, Bitmap value) {
        ...
        if (oldValue==null) {
            Log.d("TUtil", "MC：存入内存：" + value + "(" + key + ")");
            Log.d("TUtil", "MC：内存缓存总大小：" + mLruCache.size() / 1024 + "KB");
        }
    }
    ...
    static Bitmap getBitmapFromMemory(String key) {
        ...
        if (bitmap!=null){
            Log.d("TUtil", "MC：从内存读取：" + bitmap + "(" + key + ")");
        }
        return bitmap;
    }
    ...
    private static void createLruCache() {
        if (mLruCache == null) {
            ...
            Log.d("TUtil", "MC：创建 LruCache：maxSize=" + maxSize / 1024 + "KB");
     ...
     * 设置最大内存缓存空间
     */
    static void setMemoryCacheSpace(int maxSpace){
        if (maxSpace>0){
